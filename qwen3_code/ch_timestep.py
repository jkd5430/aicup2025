...
def get_time(recod3,recod4,jstime,jsdit):
    ...
    for i,j in zip(recod3,recod4):
        ...
        if len(x)>1:
            for z in x:
                for zz in z:
                    ltime=jstime[j][(jsdit[j].index(zz))]
                    jsdit[j].remove(zz)
...
local=r"G:\AI_CUP\data_AI_CUP\private_result_AI_CUP\local_CN\\"
pre=""
task1=os.listdir(local+"task1_CN")
task2_json=os.listdir(local+"task2_json_CN")
...
with open(r"G:\AI_CUP\code_AI_CUP\AI_CUP_task1_code\clean_private_result_task2_CN.txt","r",encoding="utf-8") as f:
    x=f.read()
f.close()
...
# print(li)
for ii in li:
    word=opend(local+"task1_CN/"+ii+".txt")

    jsond=openj(local+"task2_json_CN/"+ii+".json")
...
with open(r"task2_answer_CN.txt","w",encoding="utf-8") as f:
    f.write(asdd)
f.close()
...
with open(r"task2_answer_CN_other.txt","w",encoding="utf-8") as f:
    f.write(eother)
f.close()
